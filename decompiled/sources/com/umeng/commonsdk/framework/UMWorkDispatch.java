...
public class UMWorkDispatch {
    ...
    private static UMNetWorkSender mSender = null;
    ...
    public static void registerConnStateObserver(UMSenderStateNotify uMSenderStateNotify) {
        if (mSender != null) {
            UMNetWorkSender.m1157a(uMSenderStateNotify);
        }
    }
    ...
    public static void sendEventInternal(Context context, int i, int i2, UMLogDataProtocol uMLogDataProtocol, Object obj, long j) {
        if (context == null || uMLogDataProtocol == null) {
            ULog.m1381d("--->>> Context or UMLogDataProtocol parameter cannot be null!");
            return;
        }
    ...
    public static void delayProcess() {
        ...
        ULog.m1381d("--->>> delayProcess Enter...");
        UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> delayProcess Enter...");
        ...
        if (appContext != null && UMFrUtils.isOnline(appContext)) {
            ...
            if (callbackFromModuleName != null) {
                try {
                    ...
                    if (jSONObject == null) {
                        UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> analyticsCB.setupReportData() return null");
                        return;
                    }
                } catch (Throwable th) {
                ...
                if (jSONObject2 != null && jSONObject3 != null && (buildEnvelopeWithExtHeader = UMEnvelopeBuild.buildEnvelopeWithExtHeader(appContext, jSONObject2, jSONObject3)) != null) {
                    try {
                        if (buildEnvelopeWithExtHeader.has("exception")) {
                            UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> autoProcess: Build envelope error code: " + buildEnvelopeWithExtHeader.getInt("exception"));
                        }
                    } catch (Throwable th2) {
                    }
                    UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> autoProcess: removeCacheData ... ");
    ...
    public static void sendDelayProcessMsg(long j) {
        ...
        if (handler.hasMessages(MSG_DELAY_PROCESS)) {
            UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> MSG_DELAY_PROCESS has exist. do nothing.");
            return;
        }
        UMRTLog.m1143i(UMRTLog.RTLOG_TAG, "--->>> MSG_DELAY_PROCESS not exist. send it.");
        Message obtainMessage = mTaskHandler.obtainMessage();
    ...
    private static synchronized void init() {
        synchronized (UMWorkDispatch.class) {
            ULog.m1381d("--->>> Dispatch: init Enter...");
            try {
                if (mNetTask == null) {
                    ...
                    if (mTaskHandler == null) {
                        mTaskHandler = new Handler(mNetTask.getLooper()) {
                            /* class com.umeng.commonsdk.framework.UMWorkDispatch.HandlerC01571 */
            ...
            ULog.m1381d("--->>> Dispatch: init Exit...");
        }
    }
    ...
    public static void handleEvent(Message message) {
        ...
        if (callbackFromModuleName != null) {
            ULog.m1381d("--->>> dispatch:handleEvent: call back workEvent with msg type [ 0x" + Integer.toHexString(i) + "]");
    ...
    public static void handleQuit() {
        if (mSender != null && mNetTask != null) {
            UMNetWorkSender.m1162c();
            ULog.m1381d("--->>> handleQuit: Quit dispatch thread.");
            mNetTask.quit();
            teardown();
        }
    }
